...

{
	...
	printf("%d, %f, %f, %p, %p, %lf\n", mode, *((float*)alpha), *((float*)beta), x, y, epsilon);
	...
	if (idx < 2) {
		...
		*(void **)(&function) = dlsym(RTLD_NEXT, "cudnnBatchNormalizationForwardInference");
		...
		status = (*function)(handle, mode, alpha, beta, xDesc, x, yDesc, y, bnScaleBiasMeanVarDesc, bnScale, bnBias, estimatedMean, estimatedVariance, epsilon);
