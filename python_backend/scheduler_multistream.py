...
class Scheduler:
    ...
    def __init__(self, num_clients, device, barrier, policy: Optional[str] = None):
    ...
    def coschedule(self, op0, pr0, sm0, cnt0, id0, op1, pr1, sm1, cnt1, id1):
        ...
        #print(cnt0+1, op0._op, cnt1+1, op1._op)
        #print("----- coschedule ", pr0, pr1, ", hprun is: ", self._hprun)
    ...
    def schedule_one(self, index, op, cnt, col_id):
        ...
        if self._hprun == index:
            self._hp_event.wait(stream)
            
        if col_id > 0:
            self._lp_events[col_id].wait(stream)
    ...
    def schedule_blocks(self, op0, cnt0, id0, op1, cnt1, id1):
        ...
        if p0 > -1 and p0==p1:
            self.schedule_one(id0, op0, cnt0, id1)
            return None, op1

        # different profiles
        if sm0 < self._max_sms and  sm1 < self._max_sms:
            self._dur_pairs += max(d0, d1)
            self.coschedule(op0, 0, sm0, cnt0, id0, op1, 0, sm1, cnt1, id1)

        elif sm0 >= self._max_sms and sm1 < self._max_sms:
            self._dur_pairs += max(d0, d1)
            self.coschedule(op0, 0, sm0, cnt0, id0, op1, 1, sm1, cnt1, id1)

        elif sm0 < self._max_sms and sm1 >= self._max_sms:
            self._dur_pairs += max(d0, d1)
            self.coschedule(op0, 1, sm0, cnt0, id0, op1, 0, sm1, cnt1, id1)
    ...
    def schedule_profile(self):
        ...
        while True:
            ...
            if next_op1 is None and client1 > 0:
            ...
            if next_op0 is not None and next_op1 is not None:
                ...
                self.schedule_one(client0, next_op0, self._executed_calls[client0], client1)
                next_op0 = None

            elif next_op1 is not None:
                self.schedule_one(client1, next_op1, self._executed_calls[client1], client0)
                next_op1 = None
