...
class Scheduler:
    ...
    def populate_info(self, prof_file):
        ...
        self._layers_info = list(zip(df['Num_blocks'], df['Prof'], df['Mem_footprint'], df['SM']))
    ...
    def coschedule(self, op0, pr0, sm0, op1, pr1, sm1):
        ...
        #print("-------- coschedule ", op0._op, op1._op, sm0, sm1, pr0, pr1)
    ...
    def schedule_blocks(self, op0, cnt0, op1, cnt1):
        ...
        start = time.time()
        b0, p0, m0, sm0 = self._layers_info[cnt0]
        b1, p1, m1, sm1 = self._layers_info[cnt1]
        ...
        if sm0 < self._max_sms and  sm1 < self._max_sms:
            self.coschedule(op0, 0, sm0, op1, 0, sm1)

        elif sm0 >= self._max_sms and sm1 < self._max_sms:
            self.coschedule(op0, 0, sm0, op1, 1, sm1)

        elif sm0 < self._max_sms and sm1 >= self._max_sms:
            self.coschedule(op0, 1, sm0, op1, 0, sm1)
        ...
        while True:
            ...
            if next_op0 is not None:
                if next_op1 is not None:
                    ...
                    #print(next_op0._op, next_op1._op)
            ...
            if len(executed) == self._num_streams:
                # make sure everything has finished
                #if self._executed_calls[0] == self._max_calls:
                ...
                #print(self._op_queues[1].qsize())
                #while not self._op_queues[1].empty():
                #    self._op_queues[1].get()
